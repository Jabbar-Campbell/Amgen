...
#is functionalized
  ...
  ungroup() %>% 
 filter(.,TYPE == "iPS Neuron" & Cell_Line == "10") %>%
...
df4<-df3%>% 
  group_by(.,TYPE,lipid,Treatment,Cell_Line) %>% 
  filter(.,Treatment == 'DMSO' & TYPE == "iPS Neuron") %>% 
  summarize(avg_PI_norm_DMSO = mean(Pi_normalized) ) %>% 
  merge(.,df3, by = c("TYPE","lipid","Cell_Line")) 
  
df4<-mutate(filter(df4, TYPE =="iPS Neuron"), percent_to_DMSO=  (Pi_normalized/avg_PI_norm_DMSO*100))
...
output_name<-paste0( "C:/Caraway Project/RAW DATA/", substr(FILE, start = 29, stop = 35),"_output" ,".xlsx")
