...
library(dplyr)
#read in the data
df<-read.table(file="H:/ENZYMOLOGY/ENVISION/raw_data/old trfret runs/20190612___ck1_compression_quaddrants_failed_UH044122_1_001.csv",skip=11,sep = ",",nrows=32)
...
plate<-c(rep("8000198295",1152), rep("8000198284",384))
...
p38<-spread(p38, key = "column", value = value)
 ...
 #write out the files
 write.table(alpha[,c(1,4:27)],file = paste("H:/ENZYMOLOGY/ENVISION/raw_data/R_writeout_alpha_",alpha$plate[1]), row.names = FALSE, sep = "\t")
 write.table(delta[,c(1,4:27)],file = paste("H:/ENZYMOLOGY/ENVISION/raw_data/R_writeout_delta_",delta$plate[1]), row.names = FALSE,  sep = "\t") 
 write.table(gamma[,c(1,4:27)],file = paste("H:/ENZYMOLOGY/ENVISION/raw_data/R_writeout_gamma_",gamma$plate[1]), row.names = FALSE,  sep = "\t")
 write.table(p38[,c(1,4:27)],file = paste("H:/ENZYMOLOGY/ENVISION/raw_data/R_writeout_p38_",p38$plate[1]), row.names = FALSE, sep = "\t") 
