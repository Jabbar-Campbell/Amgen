...
##read in core data
x<-readxl::read_xlsx(path = FILE5, sheet = 1, skip = 1)
x<-as.data.frame(x)
class(df)
...
#is functionalized
  ...
  ungroup() %>% 
 filter(.,TYPE == "Cells" & Cell_Line == "24h") %>%
  pivot_wider(.,id_cols = c("Treatment_rep","lipid"), names_from = Treatment_rep, values_from = percentage)
...
prism_raw_function<-function(data_frame,type,Cell){
  prism<-mutate(df2,Pi_normalized = value/`Phosphate (Pi) Normalization [nmole/sample]`)%>% 
    ...
    pivot_wider(.,id_cols = c("Treatment_rep","lipid"), names_from = Treatment_rep, values_from = Pi_normalized)
...
df4<-df3%>% 
  group_by(.,TYPE,lipid,Treatment,Cell_Line) %>% 
  filter(.,Treatment == 'DMSO' & TYPE == "Cells") %>% 
  summarize(avg_PI_norm_DMSO = mean(Pi_normalized) ) %>% 
  merge(.,df3, by = c("TYPE","lipid","Cell_Line")) 
  
df4<-mutate(filter(df4, TYPE =="Cells"), percent_to_DMSO=  (Pi_normalized/avg_PI_norm_DMSO*100))
...
write_xlsx(x = output, path = "C:/Caraway Project/RAW DATA/test2_prism.xlsx", col_names = TRUE)
