...
##read in core data
x<-readxl::read_xlsx(path = FILE4, sheet = 1, skip = 1)
x<-as.data.frame(x)
class(df)
...
per_DMSO_in_media<-df2 %>% 
  filter(.,TYPE == "MEDIA") %>% 
  mutate(., percentage = value/avg_value *100)
...
#is functionalized
 #  prism<-df4%>% 
 #  ungroup() %>% 
 #  filter(.,TYPE == "Fibroblasts" & Cell_Line == "GM08399") %>%
 #  pivot_wider(.,id_cols = c("Treatment_rep","value","lipid"), names_from = Treatment_rep, values_from = value)
...
prism_DMSO_function<-function(data_frame,type,Cell){
  prism2<-per_DMSO_in_media%>% 
    ungroup() %>% 
    filter(.,TYPE == type & Cell_Line == Cell  ) %>%
    pivot_wider(.,id_cols = c("Treatment_rep","percentage","lipid"), names_from = Treatment_rep, values_from = percentage)
...
prism_normalized_function<-function(data_frame,type,Cell){
  prism3<-df4%>% 
    ungroup() %>% 
    filter(.,TYPE == type & Cell_Line == Cell  ) %>%
    pivot_wider(.,id_cols = c("Treatment_rep","percent_to_DMSO","lipid"), names_from = Treatment_rep, values_from = percent_to_DMSO)
...
df4<-df3%>% 
  group_by(.,lipid,Treatment,Cell_Line,TYPE) %>% 
  filter(.,Treatment == 'DMSO' & TYPE == "Fibroblasts") %>% 
  summarize(avg_PI_norm_DMSO = mean(Pi_normalized) ) %>% 
  merge(.,df3, by = c("TYPE","lipid","Cell_Line")) 
  
df4<-mutate(filter(df4, TYPE =="Fibroblasts"), percent_to_DMSO=  (Pi_normalized/avg_PI_norm_DMSO*100))
...
what2<-what2[,sort(names(what))]

what3<-bind_rows(empty3)
what3<-what3[,sort(names(what))]



output<-lapply(ls(pattern="what"), function(x) get(x))##FINAL OUTPUT FILE

write_xlsx(x = output, path = "C:/Caraway Project/RAW DATA/test_prism.xlsx", col_names = TRUE)##WRITE TO EXCEL THIS PACKAGE DOES REQUIRE JAVA DEPENDENTCY
